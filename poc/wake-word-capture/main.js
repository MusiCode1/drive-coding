...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const models = {
        'alexa': { url: 'assets/models/alexa_v0.1.onnx', session: null, scores: new Array(50).fill(0) },
        'hey_mycroft': { url: 'assets/models/hey_mycroft_v0.1.onnx', session: null, scores: new Array(50).fill(0) },
        'hey_jarvis': { url: 'assets/models/hey_jarvis_v0.1.onnx', session: null, scores: new Array(50).fill(0) },
    };
    ...
    async function loadModels() {
        ...
        try {
            [melspecModel, embeddingModel, vadModel] = await Promise.all([ ort.InferenceSession.create('assets/models/melspectrogram.onnx', sessionOptions), ort.InferenceSession.create('assets/models/embedding_model.onnx', sessionOptions), ort.InferenceSession.create('assets/models/silero_vad.onnx', sessionOptions) ]);
        ...
        while (mel_buffer.length >= 76) {
            ...
            for (const name in models) {
                ...
                if (score > 0.5 && isSpeechConsideredActive && !isDetectionCoolingDown) {
                    ...
                    // --- CAPTURE HOOK (added for the capture POC) ---
                    // Notify external capture logic that a wake word fired.
                    if (window.onWakeWord) window.onWakeWord(name, score);
                }
            }
    ...
    async function startListening() {
        ...
        try {
            ...
            workletNode.port.onmessage = async (event) => {
                ...
                // --- CAPTURE HOOK (added for the capture POC) ---
                // Hand every raw 1280-sample frame to the external capture logic,
                // which decides on its own when to accumulate (between wake words).
                if (window.onAudioChunk) window.onAudioChunk(chunk);
    ...
    async function runWavTest_DirectLogic() {
        ...
        try {
            const resp = await fetch('./assets/hey_jarvis_11-2.wav');
