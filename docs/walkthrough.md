## 2026-06-03 — slice rtl-ltr-bidi הושלם — 5 commits

### מה בוצע?

Slice `rtl-ltr-bidi` — תמיכה דו-כיוונית מלאה (he RTL / en LTR). 5 commits.
...
**שינויים**: settings.svelte.ts, i18n.svelte.ts, +layout.svelte, keys.ts, he.ts, en.ts, LanguageSelect.svelte, SettingsScreen.svelte, lint script חדש, package.json.
**חריגות**: אין.
**בדיקות**: typecheck ✓, lint:i18n ✓, lint:rtl ✓ (positive + negative test). Manual: ייבדק ע"י calev.
