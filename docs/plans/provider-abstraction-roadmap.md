...
במקום ש-`agent-session` יפרק `SessionNotification` של ACP ישירות, ה-VM צורך `SessionEvent` מנורמל:
...
> (`SessionEvent`: message/thought/user + tool_call/update) מנורמל ב-**P1**.
> נתיב ה-**session config** (`SessionConfigOption`/`SessionModeState`/`SessionModelState`
> → `ConfigOption`/`ModeState`/`ModelState`) מנורמל ב-**P2**, יחד עם ה-`AgentProvider`
...
במקום `AcpClient`, ה-VM תלוי ב-`AgentProvider` אגנוסטי:

```ts
// core/src/protocol/provider.ts (חדש)

export type ProtocolKind = "acp" | "codex-app-server" | "claude-native"  // נוסף בהדרגה
...
// onEvent מקבל SessionEvent מנורמל (לא SessionNotification של ACP)
...
export function createProvider(protocol: ProtocolKind, transport, onEvent, opts): Promise<AgentProvider>
...
`CliSpec` (ב-`core/src/schemas/agent.ts`) מקבל שדה `protocol: ProtocolKind` (ברירת מחדל `"acp"` — כל הרשומות הקיימות נשארות זהות). ה-FE קורא את ה-`protocol` של ה-cliKind שנבחר ומעביר ל-`createProvider`.

### זרימה אחרי ההפשטה

```
CLI (פרוטוקול P) → stdout → BE pipe (bytes) → WS → FE WsAcpTransport
  → createProvider(P, transport, onEvent) → AgentProvider
  → onEvent(SessionEvent מנורמל) → AgentSession בונה Bubble[]   (ללא ידע על P)
```
...
|---|-------|------|------|-----------|----------|
| **P1** | **normalized-events** | מודל `SessionEvent` מנורמל (streaming בלבד) + `acp-mapper` טהור (TDD). `agent-session.#onSessionUpdate` צורך `SessionEvent`. `SessionNotification` יורד מ-`agent-session` (נשאר רק תחת mapper/client). **לא נוגע ב-SessionOptionsPanel ולא ב-session config.** אפס שינוי התנהגות. | — (base=dev) | 7/10 | heavy |
| **P2** | **provider-interface** | `AgentProvider` interface + `createProvider` registry + שדה `protocol` ב-`CliSpec`. `createAcpProvider` עוטף את הקיים. **+ נרמול session config** (`SessionConfig`/`ConfigOption`/`ModeState`/`ModelState`) + כתיבה-מחדש של `SessionOptionsPanel`. `agent-session` + `sessions.ts` תלויים ב-`AgentProvider`. מסיר את 3 טיפוסי ה-config מ-ACP SDK. עדיין ACP בלבד. אפס שינוי התנהגות. | P1 | 8/10 | heavy |
| **P3** | **provider-codex** (או claude-native) | מחקר פרוטוקול → מימוש ספק לא-ACP ראשון מאחורי `AgentProvider` + מיפוי events → `SessionEvent`. רשומת `CLI_SPEC` + חיווט בחירה ב-FE. **מוכיח את ההפשטה e2e.** | P2 | 8+/10 | heavy |
| **P4** | **provider-#2** | הספק השני (claude-native או codex). מחדד את ה-interface לפי הלקחים מ-P3. | P3 | 8/10 | heavy |
...
3. **ה-BE נשאר byte-pipe אגנוסטי** — אין client של פרוטוקול ב-BE. כל ספק רץ ב-FE מעל `WsAcpTransport`. (עקבי עם D6 + הארכיטקטורה הקיימת.)
4. **`protocol` כברירת-מחדל `"acp"`** ב-`CliSpec` — opt-in, אפס רגרסיה לרשומות קיימות.
...
| כלל זהב #5 (אין backward-compat-in-place): אסור לתחזק `SessionNotification` ו-`SessionEvent` במקביל | כל slice ממיר consumer במלואו באותו commit. ACP types נשארים רק מתחת ל-mapper/provider. |
| ספק לא-ACP עלול לחשוף שדות שאין להם מקבילה מנורמלת (למשל plan/available_commands) | ה-`SessionEvent` union ניתן להרחבה; P3 יוסיף variants לפי הצורך. אין לחסום את P1 על שלמות עתידית. |
...
1. **P1 brief**: `docs/plans/slice-provider-1-normalized-model.md` (נכתב, ממתין לאימות אביגיל → dispatch לאליעזר).
2. אחרי GO על P1 — כתיבת P2 brief.
3. לפני P3 — מחקר פרוטוקול (codex app-server / claude native wire format) + החלטה איזה ספק ראשון.
