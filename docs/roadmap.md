...
| **virtualization / ביצועי גלילה** — `ChatBubbles.svelte` עושה `{#each bubbles}` ללא windowing → כל הבועות ב-DOM בו-זמנית → איטיות בשיחה ארוכה. smart-scroll כבר מוזג (`slice-mode-label-scroll`) אבל windowing לא. צריך virtual list עם שימור anchoring + auto-follow + jump-to-bottom (reference: CodeNomad `virtual-follow-list.tsx`). עצמאי לחלוטין. | ✅ **brief READY** (`slice-chat-virtualization.md`; אביגיל r1→r3; **virtua** + Option B (AppShell נשאר scroll-owner, `scrollRef`) + ResizeObserver + user-intent; complexity 8 → calev-heavy + phase-check. dispatch אחרי אישור) |
