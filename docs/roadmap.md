...
| **B — `markdown-dir-per-paragraph`** | #6 `dir="auto"` פר block-element | 🟡 **בוצע — כלב GO 8/8, ממתין smoke+merge** (2026-06-29). **merge-order ההיסטורי "B לפני D" בטל** — D כבר על dev; B נבנה מעל dev+D (אביגיל אימתה-מחדש `f1763d4`, 6/6 claims, finding 🟢 `dir`-guard שולב). הרחבת ה-DOMPurify hook הקיים: `BIDI_BLOCK_TAGS`→`dir="auto"` עם guard `!hasAttribute("dir")`; `pre/code/span` מודרים (קוד LTR). branch `slice/markdown-dir-per-paragraph` (`0fe7b87`), 67/67 tests (7 חדשים), typecheck 0. |
