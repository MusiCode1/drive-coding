...
| **הזרקת prompt-מערכת מותאם-אודיו פר-CLI** — הנחיות לעוזר voice-first מוזרקות לכל ספק בדרכו (claude=CLI flag, opencode=plugin, codex=app-server prompt). תשתית להתאמת התנהגות-קול פר-מנוע. | 💭 **מחקר-מנגנונים מלא** (opencode כבר ממומש) — slice A (codex+qoder) / B (gemini) / C (claude). ר' `docs/investigations/2026-06-19-cli-prompt-injection-mechanisms.md` |
| **ניקוי/ארגון `packages/provider` (post-cutover)** — אחרי ה-cutover נשארו תיקיות-רפאים, פיצול-spawn משולש, ושני shapes ל-capabilities. עיצוב-יעד: 6 מהלכים; **`normalize.ts` חיוני** לתיקון warm-reattach (raw↔normalized capabilities). | 💭 **עיצוב-יעד קיים, טרם slice** — ר' `docs/investigations/2026-07-01-provider-package-organization.md` |
...
| **הדבקת תמונות (paste→preview→send)** — `sendPrompt(text)` טקסט-בלבד; צריך multimodal `PromptContent[]` + `onpaste`/drop/picker + preview + דחיסה. שורש מאומת: ה-FE מדבר ACP ישירות דרך `AcpClient.prompt(sessionId,text)` (text-only, client.d.ts:45); ה-`PromptContent` הקנוני כבר מולטימודלי (events.d.ts:160) אך השכבה הזו עוד לא. BE=dumb-pipe (0 שינוי). | 🟡 **Commits 0–3 מוזגו ל-dev** (merge `2cdb85a`; פיגום רדום `IMAGE_INPUT_ENABLED=false`). **Commit 4 — ה-gate החיצוני נעלם** אחרי provider cutover v0.8.0 (`packages/provider/` נספג → קוד שלנו; אין יותר "סוכן שני"). הפך ל-slice עצמאי. **חסום כעת רק על הכרעת-gating אחת** — raw (`#client.capabilities.promptCapabilities.image`, עובד היום) מול normalized (`NormalizedCapabilities` שאין בו `image`) — מופנית לסוכן ה-provider-cutover. ר' `slice-image-paste.md` §10. |
...
| **מילת-התעוררות בצד-לקוח (wake word)** — היום push-to-talk; ליבת hands-free = wake-word. נסקרו 5 מנועים; **PoC בוצע וקיים ב-git** (מסלול C-new עם `openwakeword-wasm-browser`, "Hey Jarvis" ~יום-עבודה). עברית = slice 17 (`.onnx` ייעודי). | 💭 **PoC מאומת** — ר' `docs/investigations/wake-word-client-side.md` + `slice-17-wake-word.md` / `wake-word-fe-design.md` |
| **מחיקת סשנים** — RFD ב-ACP Preview; **דחוי עד שדרוג SDK+CLI**. (עריכת-שם לא קיימת בפרוטוקול → נזנחת מצד-משתמש.) | 💭 **דחוי (תלוי-upstream)** — ר' `docs/investigations/2026-06-03-session-delete-rename.md` |
| recordings + replay | 💭 |
...
| **הרצת drive-coding ב-Termux/Android** — codex נכשל ב-spawn (npx resolution), claude נכשל ב-SDK; שורש מאומת + **פתרון עוקף בקונפיג בלבד** (`cli-specs.jsonc` + `.env`, ללא שינוי-קוד). מרחיב את הגישה ההיברידית לנייד-אמיתי (לא רק דפדפן). | 💭 **שורש מאומת, פתרון-config** — ר' `docs/investigations/2026-07-01-termux-android-cli-spawn.md` |
...
| **בילד FE בלי ריסטארט (decouple)** — לבנות מחדש את הפרונט בלי לעצור את הסוכנים הרצים; ריסטארט רק על שינויי-BE (היום ריסטארט עוצר את כל הסוכנים). | ✅ **מוזג ל-dev** (merge `bc3688a`; `slice-fe-build-decouple.md`→archive; 4 commits: פרמטריזציית `outDir` ב-svelte.config + סקריפט `dc-build-fe.mjs` + aliases `fe:build`/`fe:build:if-missing` + `ExecStartPre` build-if-missing בשני ה-units; build-gate ירוק `1e06edd`) |
...
| **חוסן כיבוי-BE + פורטים שלא משתחררים** — נתפס חי 2026-07-01 (כאב חוזר לפי המשתמשת). ה-BE נתקע (מאזין אך HTTP `http=000`, ~60 סוקטים חצי-סגורים), סגירת-טרמינל לא הרגה אותו, והפורט **לא השתחרר גם אחרי מות התהליך** (handle-inheritance; הריגת בני-codex לא עזרה). 5 פערים: (1) אין `SIGINT`/`SIGTERM`/graceful-shutdown (רק uncaughtException/unhandledRejection ב-`server.ts`); (2) event-loop נתקע; (3) listen-socket עובר בירושה → פורט ננעל; (4) תהליכי-בן codex-acp יתומים לא מתנקים; (5) codex boot ~10ש' (`npx @latest`) מתנגש ב-`INIT_TIMEOUT_MS=10_000` → flaky connect. | 🟢 **brief READY r2** (`slice-be-shutdown-hardening.md`, אביגיל READY r2, Complexity 7/light; base=dev, depends_on=[]). **עבר repro-אמיתי** (§repro-ג בחקירה): הבאג אומת חי (67512 תפוס יומיים), אבל **BE בריא עמיד** — 3 תרחישי-כיבוי + 3 ניסויי-stress (WS/spawn) לא חנקו; bun מנקה happy-path. **מסקנה: הצטברות-סוקטים=סימפטום של hang, לא סיבה; שורש-hang לא שוחזר.** 3 commits: **kill-tree** (POSIX group + Windows `taskkill /T`, תבנית CodeNomad) + **graceful-shutdown** = **defense-in-depth** · **`lastPingAt`+sweep** (על ה-`$/ping` שה-FE כבר שולח) = **ניקוי-קליינט-מת** (לא תרופת-hang). טרם dispatch. |
| **`be-hang-supervisor` — התאוששות מ-hang (watchdog חיצוני)** — נגזר מ-repro של be-shutdown-hardening: ה-BE הבריא עמיד, אבל כשהוא **כן** נתקע (event-loop block, לא שוחזר) — kill-tree/graceful-shutdown רצים *מתוך* ה-loop התקוע → **לא ירוצו**. הפתרון: **watchdog חיצוני-ל-loop** שמודד ping round-trip (או health-probe), ועל אי-מענה → kill-tree מבחוץ. **agnostic לסיבת-ה-hang** (לא דורש לדעת שורש). תבנית: CodeNomad `cli-supervisor.cjs`. משנה מבנה-הרצה (supervisor מעל BE). | 💭 **טרם brief** — נגזר 2026-07-02; נבדק עם hang-מוזרק (busy-loop endpoint). ר' `docs/investigations/2026-07-01-be-shutdown-socket-health.md` §repro-ג + §השלכה. **הרחבת-scope (2026-07-04, מדיון `proxy-tap-memory`):** ה-supervisor צריך לכסות **גם RSS/memory, לא רק hang** — ה-`memoryGuard` שנוסף ב-`proxy-tap-memory` (Commit 3) הוא **in-process** ולכן חלקי: תופס רק **טיפוס-הדרגתי** של RSS, ונופל בשני התרחישים הקריטיים — **OOM-native** (התהליך מת לפני שה-`setInterval` רץ) ו-**event-loop-hang** (ה-poll יושב באותו loop תקוע). ניתוח האלטרנטיבות: **worker-thread** שורד hang (event-loop נפרד) אך **מת ב-crash/OOM** (אותו OS process); **child-process** — התבנית הנכונה **supervisor=parent · BE=child** — שורד את **שניהם**, רואה RSS דרך ה-OS (pid query, כמו `scripts/watch-be-rss.sh`), ויכול לנקוט פעולה מבחוץ (kill-tree/restart). זה הפתרון היחיד ל-blast-radius (**המארח + כל הסוכנים ה-in-process שתלויים בו**). כשיֵצא ה-slice: **לספוג את ה-`memoryGuard` ה-in-process** (ה-503-degradation) לתוך ה-supervisor. |
| **איחוד מקורות-אמת של agent-state (NBug2)** — היום מצב-הסוכן מפוזר ב-4 מקורות → חוסר-עקביות ו-NBug2. עיצוב-יעד: `AgentLifecycleManager` יחיד. **אופציה B** (איחוד תחת `connected` בלבד, סיכון-נמוך) מומלצת כשלב-ראשון לפני האיחוד המלא. | 💭 **עיצוב קיים, טרם brief** — ר' `docs/investigations/2026-06-04-agent-lifecycle-A-design.md` + `-single-mechanism.md` |
| **Multi-client mux — N לקוחות מול CLI אחד** — שני דפדפנים/מכשירים על אותו session במקביל. הליבה הקשה = **id-NAT ב-BE** (מיפוי request-id פר-לקוח). מסלול 4-slices. **מכליל את "WS thrashing (2-tabs)"** לפתרון-שורש במקום workaround. | 💭 **תוכנית קיימת, לא מאושר** — ר' `docs/investigations/2026-06-26-multi-client-mux-design.md` |
| **דליפת-זיכרון ב-build/typecheck (OOM)** — `vite build` דולף ~1.38GB (מדוד) → OOM אקראי ב-gate. checkers חלופיים (`svelte-check-rs`/native) מהירים ×28–77 אך false-positives → **לא** להחליף ב-gate. כיוון מאומת: **rolldown-vite −21% RAM**. | 💭 **ממצאים מדודים, טרם slice** — ר' `docs/investigations/2026-06-27-build-check-memory-rust-tooling.md` |
| **F4 — תגובה מקוטעת אחרי reload** — כשה-FE מתנתק באמצע turn, ה-BE לא מ-buffer את ה-chunks → אחרי reload התגובה חסרה. **הכרעה ארכיטקטונית פתוחה**: BE buffer / accept-loss / cancel-on-disconnect. (F3 — user-bubbles נעלמות — כבר תוקן ב-slice 8.1.) | 💭 **הכרעה פתוחה** — ר' `docs/investigations/f3-f4-session-reload-bubbles.md` |
