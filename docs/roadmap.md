...
| **C — `code-copy-button`** | #2 כפתור-העתקה פר code-block | 🟡 **בוצע — כלב GO 8/8, ממתין אישור merge** (2026-06-29). **הופרד מהשרשרת**: depends_on:[A] מומשה (A מוזג `a20fbda`) → base=dev ישיר; עצמאי-קבצים מ-B/D. branch `slice/code-copy-button` (`f14ec41` action + `8957af3` חיווט+CSS). Svelte action `enhanceCodeBlocks` co-located + event-delegation (גוטשת-streaming אומתה חי ע"י כלב); FE-only (`bubble.copy/copied`), typecheck 0, 371/371. אביגיל r2 READY. |
