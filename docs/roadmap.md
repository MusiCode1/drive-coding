...
| **הדבקת תמונות (paste→preview→send)** — `sendPrompt(text)` טקסט-בלבד; צריך multimodal `PromptContent[]` + `onpaste`/drop/picker + preview + דחיסה. שורש מאומת: ה-FE מדבר ACP ישירות דרך `AcpClient.prompt(sessionId,text)` (text-only, client.d.ts:45); ה-`PromptContent` הקנוני כבר מולטימודלי (events.d.ts:160) אך השכבה הזו עוד לא. BE=dumb-pipe (0 שינוי). | 🟢 **brief READY** (`slice-image-paste.md`, Complexity 8 → calev-heavy; אביגיל r2 READY; decision נרשם). 4 commits: 0–3 (core resize-plan TDD + FE engine + TypeArea capture/tray/gating + UserBubble render) **בְּני-ביצוע מיידי**; **Commit 4 (שליחה מולטימודלית) GATED + merge מוקפא** עד ש-`AcpClient.prompt` יקבל blocks (Track A, הסוכן השני). gating חובת-spec דרך `promptCapabilities.image` |
...
| **ממשק אישור-בקשות (permission UI)** — כיום `session/request_permission` מאושר אוטומטית כברירת-מחדל, אין UI להחליט. צריך: UI אישור/דחייה לכלי + מיפוי אילו modes (bypass) בכלל לא שולחים בקשה. קשור ל-F "הריצה נעצרת". | 💭 **טרם brief** — נדון ב-fdf80659/e038a47a. **שלב-מקדים נעשה ב-`leave-running-background`** (ראה Track F): זיהוי-bypass רוכז ב-`permission-mode.ts` (claude בלבד) עם הערת-קוד לאיחוד חוצה-ספקים כשיושלם מנגנון-ה-ACP המאוחד |
