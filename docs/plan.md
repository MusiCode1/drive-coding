...
### ג. באגים ב-`config.html` שדווחו ע"י Avi (2026-05-14)

קבוצת תיקוני UX קטנים בעמוד ההגדרות. ככל הנראה משימות אטומיות שאפשר לאחד למשימה אחת (R) או לפצל. צריך תכנון לפני העברה ל"לביצוע".

1. **כותרת הסשן לא מוצגת** — בעמוד הראשי או ב-config (לאמת איפה בדיוק) — שם/כותרת הסשן הנוכחי חסרה. צריך להציג אותה בולטת בראש העמוד.
2. **חסר state של "טוען..."** — כשהעמוד מאחזר נתונים (models, sessions, וכו') אין אינדיקציה ויזואלית. נראה ריק / שבור.
3. **טעינה אוטומטית של פרטי הסשן** — היום צריך לחיצה ידנית על "טען אפשרויות" (כבר תועד חלקית ב-`future-features.md` סעיף 16b). להפוך לאוטומטי לפחות עבור cwd שמור ב-localStorage.
4. **תיקיית בית כברירת מחדל** — בפתיחה, שדה ה-cwd ריק במקום להציג את `$HOME`. צריך ב-default value.
5. **עץ התיקיות נכשל עם "מחוץ לנתיב"** — ניסיון לפתוח את ה-picker לעיון בתיקיות מחזיר שגיאה. נראה כמו validation מחמיר מדי ב-backend (`/api/browse` או דומה) שמסרב לעבור מעל cwd ההתחלתי.

**צעדים בתכנון:**
- לקרוא את `frontend/config.html` ואת ה-endpoints הרלוונטיים ב-`backend/src/server.ts`.
- לאתר את הולידטור של ה-picker ולהבין למה הוא בולם.
- להחליט אם זו משימה R אחת ("תיקוני UX של config") או 5 משימות נפרדות.
- לתאם עם משימה Q שעדיין ממתינה — אם המבצע יקח את שתיהן ברצף, סדר מומלץ: Q (frontend חדש) ואז R (תיקוני config).

**עדיפות:** בינונית — לא חוסם, אבל פוגע ב-onboarding של משתמש חדש.

**ממתין להחלטת ה-planner השני** (שמתכנן ריפקטור v6 עכשיו) — האם לשלב את התיקונים בתוך v5/Q או להגדיר v5.5 נפרדת.
