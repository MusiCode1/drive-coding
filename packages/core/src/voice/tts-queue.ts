...
export class OrderAllocator {
  ...
  /**
   * מנקה את מצב הבועות. ה-seq הגלובלי **לא** מתאפס (מונוטוניות בין שיחות).
   *
   * 🔴 **אינו נקרא עוד מ-`Speaker.stop()`** — ר' ההערה שם. השארתי אותו
   * לשימושי-בדיקה ולניקוי-מלא אמיתי (החלפת סשן), אבל ניקוי אגב-עצירה
   * דחף בועות ישנות **לסוף** תור-ההשמעה.
   */
  clear(): void {
    this.#bubbles.clear()
  }
