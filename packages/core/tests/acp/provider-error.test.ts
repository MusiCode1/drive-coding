...
import { extractProviderError } from "../../src/acp/provider-error.js"
...
describe("extractProviderError — pattern 1: JSON message", () => {
  ...
  test("invalid API key — keyword 'invalid'", () => {
    const lines = [`{"error":{"message":"Invalid API key provided. Check your authentication."}}`]
    expect(extractProviderError(lines)).toBe("Invalid API key provided. Check your authentication.")
  })

  test("rate limit — keyword 'rate'", () => {
    const lines = [`{"message":"Rate limit exceeded for this account."}`]
    expect(extractProviderError(lines)).toBe("Rate limit exceeded for this account.")
  })

  test("unauthorized — keyword 'unauthor'", () => {
    const lines = [`{"message":"Request unauthorized. Please re-authenticate."}`]
    expect(extractProviderError(lines)).toBe("Request unauthorized. Please re-authenticate.")
  })

  test("message without a keyword is NOT returned", () => {
    const lines = [`{"message":"Random log message about nothing in particular."}`]
  ...
  test("returns the MOST RECENT match (scans newest-first)", () => {
    ...
    expect(extractProviderError(lines)).toBe("Latest credit error to report.")
  })
})

describe("extractProviderError — pattern 2: opencode ERROR log", () => {
  test("ERROR line with error= field", () => {
    const lines = ["INFO starting", "ERROR provider failed error=Could not connect to upstream"]
    expect(extractProviderError(lines)).toBe("Could not connect to upstream")
  })

  test("ERROR with stack= field — strips at stack", () => {
    const lines = ["ERROR module=acp error=Stream closed unexpectedly stack=at foo:1"]
    expect(extractProviderError(lines)).toBe("Stream closed unexpectedly")
  })
  ...
  test("ERROR pattern is fallback — pattern 1 has priority", () => {
    ...
    const lines = [`ERROR error=plain error`, `{"message":"Invalid credentials, try again."}`]
    expect(extractProviderError(lines)).toBe("Invalid credentials, try again.")
  })

  test("ERROR pattern scans last 50 lines", () => {
    // 60 noise lines, then ERROR far back — should be missed.
    const lines = ["ERROR error=ancient error", ...Array(55).fill("INFO noise")]
    expect(extractProviderError(lines)).toBeNull()
  })
})
...
describe("extractProviderError — edge cases", () => {
  ...
  test("only noise → null", () => {
    expect(extractProviderError(["INFO a", "INFO b", "WARN c"])).toBeNull()
  })
