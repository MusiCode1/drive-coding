 ...
 * 🔴 **‏חייב להיות גדול מאורך ה-cue עצמו.** ‏ב-`cues.ts`: ‏`speaking` ‏הוא
 * ‏`#playGlide(…, 300)` — ‏הגיין מגיע ל-0 ‏ב-300ms ‏והצומת נעצר ב-350ms; ‏`thinking`
 * ‏הוא שני `#playPluck` ‏(‏השני ב-70ms) ‏והצומת האחרון נעצר ב-320ms.
