...
describe("AudioPlaylist — יתומים, וסדר-ההשמעה", () => {
  ...
  it("skip-telemetry: יתום ממוין-מוקדם מסומן skipped עם סיבה — ולא נשאר ready", async () => {
    const sink = makeMockSink()
    const pl = new AudioPlaylist(sink, undefined, { reserveTimeoutMs: 1000 })
    ...
    // ההתנהגות לא השתנתה — הוא עדיין לא מושמע
    expect(sink.playOrder).toEqual(["later"])

    // 🔴 אבל עכשיו הוא **מתועד** במקום להישאר ready לנצח
    const it0 = pl.items.find((x) => x.segmentId === "stale")
    expect(it0?.state).toBe("skipped")
    expect(it0?.skipReason).toBe("orphan-behind-cursor")
    expect(it0?.reconsiderable).toBe(false)

    const log = pl.skipLog()
    expect(log).toHaveLength(1)
    expect(log[0]?.reason).toBe("orphan-behind-cursor")
    expect(log[0]?.bubbleId).toBe("b-stale")
    // ההקשר שמאפשר לשפוט בדיעבד אם ההחלטה הייתה נכונה
    expect(log[0]?.context.lastPlayed).toBeGreaterThanOrEqual(0)
  })

  it("skip-telemetry: המצב שנמדד במכשיר — cursor===items עם ready ⇒ אפס ready אחרי", async () => {
    const sink = makeMockSink()
    const pl = new AudioPlaylist(sink, undefined, { reserveTimeoutMs: 1000 })
    // כמו שנמדד: משהו באינדקס גבוה נוגן, ואז מגיע סגמנט שממוין לפניו
    await playOneTurn(pl, sink, "A5", 5)
    pl.reserve("orphan", { seq: 0, segmentIndex: 0 }, "b-A")
    await settle()
    await sink.prepareSegment("orphan", new ReadableStream(), new AbortController())
    pl.markReady("orphan")
    await vi.advanceTimersByTimeAsync(1500)
    await settle()

    const info = pl.debugInfo()
    // הקריטריון המדיד: כשחונים בסוף, אסור שיישאר ready
    // 🔴 לא בתוך if — התנאי עצמו הוא חלק מהטענה, אחרת הטסט עובר ריק
    expect(info.cursor).toBeGreaterThanOrEqual(info.items)
    expect(info.byState.ready ?? 0).toBe(0)
    expect(info.skipsByReason["orphan-behind-cursor"]).toBe(1)
    expect(info.recentSkips[0]).toMatch(/^orphan-behind-cursor:/)
  })

  it("skip-telemetry: timeout מסומן reconsiderable — דילוג הפיך", async () => {
    const sink = makeMockSink()
    const pl = new AudioPlaylist(sink, undefined, { reserveTimeoutMs: 500 })
    pl.reserve("never", { seq: 0, segmentIndex: 0 }, "b-t")
    await settle()
    await vi.advanceTimersByTimeAsync(1200)
    await settle()
    const it0 = pl.items.find((x) => x.segmentId === "never")
    expect(it0?.skipReason).toBe("reserve-timeout")
    expect(it0?.reconsiderable).toBe(true)
    expect(it0?.state).toBe("skipped")
  })
