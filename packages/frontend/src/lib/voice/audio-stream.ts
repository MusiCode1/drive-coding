 ...
 *
 * MED-6 (audit): Added 5s timeout on sourceopen to avoid infinite hang.
 * MIN-5 (brief): If stream is interrupted, state="cancelled", play() rejects →
 *   orchestrator advances to next segment (skip).
 *
...
export class AudioStream {
   ...
   * MIN-5: If segment is cancelled, rejects → orchestrator skips to next.
   */
  async play(segmentId: string): Promise<void> {
    ...
    // MIN-5: cancelled → reject, orchestrator skips
    if (seg.state === "cancelled") {
  ...
  private waitForReady(seg: AudioSegment): Promise<void> {
