 ...
 *
 * אל תיגע ב-Player/Speaker/AudioStream — נתיב <audio> פשוט להשמעה חד-פעמית. (§6)
 *
 * ─── slice model-status-control-replay (Commit 5) ───
 */
 ...
 * stream → Blob → objectURL → <audio>. URL.revokeObjectURL חובה אחרי ended/abort (§4.7).
 */
export async function playAgentText(
  ...
  try {
    ...
    // §4.7: חובה לשחרר objectURL אחרי ended/stop/abort
