 ...
 *
 * msr-v2 (Commit 4): הוחלפה ה-stub saveRecording בקריאה אמיתית ל-recordings.ts.
 * שמירה כושלת → try/catch מחזיר recordingId:"" (תמלול לא נופל).
 *
 * הועתק מתוך main/packages/frontend/src/lib/voice/stt-client.ts (slice 3).
 * שינויים:
 *   (a) ייבוא של saveRecording מ-"./recordings" (msr-v2 — הוסר stub)
 *   (b) slice sessions-inline: timeout 15s→30s, עטוף withRetry (3 נסיונות, backoff 800ms)
 */
...
export async function transcribe(
  ...
  // msr-v2: שמירת הקלטה ב-BE — best-effort (שמירה כושלת לא מפילה תמלול)
  const recordingPromise = saveRecording(blob).catch(() => ({ id: "" }))
