 ...
 *
 * ⚠️ **אל תוסיף שדות אופציונליים ל-`BubbleBase`.** הפיתוי הוא להרחיב הדרגתית
 * (`segments?`, `toolCall?`, `recordingId?`) כשמגיע צורך חדש — וזה בדיוק הדפוס
 * שיצר ב-FE הישן את `messages` + `bubbles` הכפולים ואת `segmentCache` שמעולם לא
 * אוכלס. הכלל: כל variant נושא את שדותיו במלואם, גם אלה שעדיין לא בשימוש; מי
 * שלא צריך אותם פשוט לא ניגש אליהם.
 */
