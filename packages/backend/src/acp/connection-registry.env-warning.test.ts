/**
 * connection-registry.env-warning.test.ts — the exported override predicates (open-connection.ts).
 *
...
// In-process connectors are claude and codex. The predicates moved to
// open-connection.ts when the sidecar route was added and the registry stopped
// deciding how an agent is hosted; the warning condition is unchanged.
...
describe("overrideHasEnv / overrideHasBinOrArgs", () => {
  ...
  it("in-process claude with setEnv → overrideHasEnv true", async () => {
    ...
    const { overrideHasEnv } = await import("./open-connection.js")
  ...
  it("regression: overrideHasBinOrArgs behaves as before", async () => {
    ...
    const { overrideHasBinOrArgs } = await import("./open-connection.js")
